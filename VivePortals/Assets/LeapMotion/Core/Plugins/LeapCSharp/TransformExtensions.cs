...
namespace Leap {
  ...
  public static class TransformExtensions {
    ...
    public static Frame TransformLeft(this Frame frame, LeapTransform transform) {
        for (int i = frame.Hands.Count; i-- != 0;) {
            if(frame.Hands[i].IsLeft) frame.Hands[i].Transform(transform);
        }
        return frame;
    }

    public static Frame TransformRight(this Frame frame, LeapTransform transform) {
        for (int i = frame.Hands.Count; i-- != 0;) {
            if(frame.Hands[i].IsRight) frame.Hands[i].Transform(transform);
        }
        return frame;
    }
    ...
    public static Frame TransformedCopyLeft(this Frame frame, LeapTransform transform) {
      return new Frame().CopyFrom(frame).TransformLeft(transform);
    }

    public static Frame TransformedCopyRight(this Frame frame, LeapTransform transform) {
      return new Frame().CopyFrom(frame).TransformRight(transform);
    }
